...
# Supplementary Plane & Unicode Emojis -> Clean BMP Vectors
EMOJI_PATTERNS = [
    # Airplanes / Flights -> ✈ (\u2708)
    (re.compile(r'[\U0001F6EB\U0001F6EC\U0001F6E9\U0001F6E8\U0001F6F0\u2708]'), '✈'),
    # Heavy Circles (red, green, blue, heavy outline circle) -> O
    (re.compile(r'[\U0001F534\U0001F535\U0001F7E0\U0001F7E1\U0001F7E2\u2B55\u26AA\u26AB]'), 'O'),
    # Fleur-de-lis / Decorative -> ❖
    (re.compile(r'[\u269C\u2740\u2741]'), '❖'),
    # Scissors -> ✂
    (re.compile(r'[\u2702\u2703\u2704]'), '✂'),
    # Stars -> ★
    (re.compile(r'[\u2B50\U0001F31F\U0001F4AB\u2605\u2606]'), '★'),
    # Phones / Helpline -> ☎
    (re.compile(r'[\U0001F4DE\U0001F4F1\U0001F4F2\U0001F919\u260E\u260F]'), '☎'),
    # Tools / Gears -> ⚙
    (re.compile(r'[\U0001F6E0\U0001F527\U0001F528\u2699]'), '⚙'),
    # Checks -> ✔
    (re.compile(r'[\u2705\u2714\u2713]'), '✔'),
    # Crosses -> ✘
    (re.compile(r'[\u274C\u274E\u2718\u2717]'), '✘'),
    # Pin / Location -> 📍
    (re.compile(r'[\U0001F4CD\U0001F525\U0001F4CE]'), '📍'),
    # Arrow / Rocket / Thumbs -> ▶
    (re.compile(r'[\U0001F680\U0001F44D\U0001F44E]'), '▶'),
]
...
def sanitize_emojis(text: str) -> str:
    ...
    if not text:
        return text
    # 1. Strip all variation selectors (\uFE00 to \uFE0F) that cause square boxes
    text = re.sub(r'[\uFE00-\uFE0F]', '', text)
    # 2. Apply emoji pattern substitutions
    for pattern, replacement in EMOJI_PATTERNS:
        text = pattern.sub(replacement, text)
    # 3. Strip any remaining high-surrogate characters that standard fonts can't render
